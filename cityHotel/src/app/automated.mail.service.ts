...
const SOLICITUD_DE_RESERVA = "Solicitud De Reserva";
const NO_REPLY_SOLICITUD_RECIBIDA = "NO-REPLY Hemos recibido su solicitud de reserva"
...
export class AutomatedMailService {
  ...
  public sendReservationRequestMail(requestInfo: any) {
      ...
      let requestBody = new SendMailRequest(this.HOTEL_MAIL, SOLICITUD_DE_RESERVA, reservationRequestMessage);
      this.sendEmail(requestBody);
      this.sendReservationReplyMail(requestInfo);
  }

  private sendReservationReplyMail(requestInfo: any) {
    ...
    let requestBody = new SendMailRequest(requestInfo.email, NO_REPLY_SOLICITUD_RECIBIDA, reservationReply);
    this.sendEmail(requestBody);
